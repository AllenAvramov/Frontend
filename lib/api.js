...
api.interceptors.request.use(async (config) => { //interceptor: to intercept before the error of expired token and get a new one
  ...
  if (config.data instanceof FormData) {
    ...
    delete config.headers['Content-Type']; // Let axios set it automatically for FormData
  } else {
    console.log('📤 API Interceptor: Not FormData, keeping Content-Type:', config.headers['Content-Type']);
  }
